...
export default async function ClubDetailLayout({
  ...
  const { data: club, error: clubError } = await supabase
    .from("clubs")
    .select("*, owner_profile:profiles!clubs_owner_id_fkey(id, username, full_name, avatar_url, tagline, certified, bio, link, tags, updated_at)")
    .eq("id", club_id)
    .single();

  const { data: members, error: membersError } = await supabase
    .from("club_members")
    .select("*, profiles(id, username, full_name, avatar_url, tagline, certified, bio, link, tags, updated_at)")
    .eq("club_id", club_id);
  ...
  return (
    <main className="flex justify-center items-start gap-x-4 pb-3 md:pb-5 min-h-screen max-w-screen-xl mx-auto">
      ...
      <div className="w-auto flex-1 border-x border-gray-200 min-w-0 min-h-screen px-0 md:px-6 lg:pr-0">
        {children}
      </div>
