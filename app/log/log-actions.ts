...
import { createSuccessResponse, CreateResponse } from "@/lib/types/api";
...
export const createLog = withAuthForm(async ({ supabase, user }, formData: FormData) => {
...
export const updateLog = withAuthForm(async ({ supabase, user }, formData: FormData) => {
  const logId = validateRequired(formData.get("logId") as string, "로그 ID");
  const content = validateRequired(formData.get("content") as string, "내용");
  const imageFile = formData.get("imageFile") as File | null;
  const imageRemoved = formData.get("imageRemoved") === "true";

  const { data: oldLogData } = await supabase.from("logs").select("image_url, user_id").eq("id", logId).single();

  if (oldLogData?.user_id !== user.id) {
    return { error: "수정할 권한이 없습니다." };
  }

  const imageUrl = await handleLogImage(logId, imageFile, imageRemoved, oldLogData?.image_url);
  const processedContent = await processMentionsForSave(content, supabase);
  ...
  const { error } = await supabase.from("logs").update(updateData).eq("id", logId);
...
export const createComment = withAuth(async ({ supabase, user }, formData: FormData) => {
...
export const updateComment = withAuth(async ({ supabase, user }, formData: FormData) => {
  const content = validateRequired(formData.get("content") as string, "댓글");
  const commentId = validateRequired(formData.get("comment_id") as string, "댓글 ID");
  const logId = validateRequired(formData.get("log_id") as string, "로그 ID");
...
export const toggleLogBookmark = withAuth(async ({ supabase, user }, logId: string, hasBookmarked: boolean) => {
  if (hasBookmarked) {
    const { error } = await supabase
      .from('log_bookmarks')
      .delete()
