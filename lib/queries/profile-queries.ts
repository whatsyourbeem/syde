...
export async function getProfileById(
  ...
  const { data, error } = await supabase
    .from("profiles")
    .select("id, username, full_name, avatar_url, tagline, bio, link, tags, updated_at, certified")
    .eq("id", userId)
    .single();
...
export async function getProfilesList(
  ...
  let query = supabase
    .from("profiles")
    .select("id, username, full_name, avatar_url, tagline, bio, link, tags, updated_at, certified", { count: "exact" });
...
export async function getProfileByUsername(
  ...
  const { data, error } = await supabase
    .from("profiles")
    .select("id, username, full_name, avatar_url, bio, link, tagline, tags, updated_at, certified")
