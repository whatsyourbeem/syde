        ...
        {isExpanded ? (
          <div className="flex flex-col gap-2 lg:gap-3">
            {displayShowcases.map((showcase, index) => (
              <div key={showcase.id}>
                <Link href={`/showcase/${showcase.slug || showcase.id}`} prefetch={false} className="block group">
                  <div className="flex items-center gap-2">
          ...
          <AnimatePresence mode="popLayout" initial={false}>
            ...
            >
              <Link href={`/showcase/${displayShowcases[currentIndex].slug || displayShowcases[currentIndex].id}`} prefetch={false} className="block group">
                <div className="flex items-center gap-2">
                  <span className="font-pretendard font-bold text-[14px] w-[14px] text-center text-sydeblue flex-shrink-0">
                    {currentIndex + 1}
                  </span>
