...
로컬 환경에서는 Google/Kakao OAuth가 동작하지 않습니다. 대신 **테스트 계정 로그인** 버튼을 사용하세요.

### 5.1 테스트 계정 로그인

로컬 서버(`npm run dev`)를 실행한 뒤 로그인 다이얼로그를 열면 하단에 **"테스트 계정으로 즉시 로그인"** 버튼이 표시됩니다.

- 이 버튼은 `NODE_ENV === "development"`일 때만 UI에 노출되며, 운영 환경에서는 표시되지 않습니다.
- 서버 액션(`loginWithTestAccount`)도 `NODE_ENV !== "development"` 시 즉시 에러를 반환하므로 직접 호출로도 우회할 수 없습니다.
- 로그인 시 사용되는 계정은 `supabase/seed.sql`에 이미 시드되어 있습니다:
  - **이메일**: `test@example.com`
  - **비밀번호**: `password123`

> [!TIP]
> `npx supabase db reset`을 실행하면 seed.sql이 다시 로드되어 테스트 계정이 복원됩니다.

### 5.2 인증이 필요한 Server Action 테스트

Server Action에는 인증 게이트(`withAuth`, `withAuthForm`)가 적용되어 있습니다.
**로그인하지 않은 상태에서 Server Action을 호출하면 `UNAUTHORIZED` 에러가 반환됩니다.**

테스트 계정으로 로그인한 뒤 기능을 테스트하세요.

> [!NOTE]
> 과거에는 `NODE_ENV === "development"` 조건으로 mock 사용자를 자동 주입하는 코드가 있었으나,
> Preview/스테이징 환경에서의 보안 취약점 위험(T-P0-01)으로 인해 제거되었습니다.
> 로컬 개발에서도 실제 인증 흐름(테스트 계정 로그인)을 사용하는 것이 원칙입니다.
...
## 10. (선택 사항) 파일 저장소 동기화 (Storage Sync)
