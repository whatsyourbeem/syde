...
| B10 | DB·스토리지 이름을 blog로 변경 | DB + 스토리지 + 코드 | 1~1.5일 | M3 (B9 다음) | ✅ b50fb1d, a56a8c9, 81f76aa, b57f7ad (6단계 대기) |
...
1. 스크립트 `scripts/migrate-blog-images.ts`(서비스 역할 키 사용, `--dry-run` 지원)를 만든다.
   ...
   - 이미 복사된 파일은 건너뛴다(다시 실행해도 안전하게). 같은 경로에 크기가 다른 파일이 있으면 덮어쓰지 않고 실패로 보고한다.
   - **합격 기준은 "원본의 모든 파일이 같은 경로·같은 크기로 대상에 있다"이다.** 배포 후 새 글 이미지가 `blog-images`에만 쌓이므로 두 버킷의 수·크기는 같지 않을 수 있다. 수·크기 일치 여부와 "대상에만 있는 파일"은 참고용으로 따로 출력한다.
   - 대상이 `127.0.0.1`/`localhost`가 아니면 `--prod` 없이는 어떤 요청도 보내지 않는다. `.env.local`이 로컬을 가리키므로, 운영에는 그 명령에서만 `NEXT_PUBLIC_SUPABASE_URL`과 `SUPABASE_SERVICE_ROLE_KEY`를 덮어써서 실행한다.
2. URL을 교체한다(한 트랜잭션).
   ...
   - 실제 구현은 `scripts/blog-images-url-replace.sql`이다. 교체 행 수·잔여 건수·`updated_at` 지문을 스스로 검증하고 어긋나면 전체를 되돌린다. 수파베이스 SQL Editor는 `NOTICE`를 보여주지 않아 "Success. No rows returned"만 뜨는데, 정상이다. 확인은 아래 3번 쿼리로 한다.
3. **확인**
   ...
   - 캐시된 상세 페이지가 옛 URL을 들고 있을 수 있으니 `blog-post-all` 태그를 한 번 무효화한다(또는 재배포). **운영에서 실제로 발생했다.** DB는 바뀌었는데 상세 페이지가 옛 주소를 들고 있었고, 재배포로 해결했다.
...
#### 6. 옛 버킷 정리 (나중에, 운영자가 직접) — ⏳ 대기 중 (2026-09-21 배포 기준 1~2주 뒤)
...
2. ✅ 마이그레이션 A + 새 코드 배포 (2~3단계, PR #20)
3. ✅ 이미지 복사와 URL 교체, 캐시 무효화를 위한 재배포 (4단계)
4. ✅ 마이그레이션 B 적용 → 호환 처리 제거 코드 배포 (5단계)
5. ⏳ 1~2주 뒤 옛 버킷 삭제와 마이그레이션 C (6단계)

**배포 후 확인 결과 (2026-09-21)**: 운영에서 호환 뷰가 사라졌고(`insights` 404), 활동 값이 모두 `BLOG_POST_CREATED`이며, 옛 버킷 주소를 쓰는 글이 0건이다. 목록·상세·글쓰기·댓글·좋아요와 피드의 블로그 활동이 정상이다.
